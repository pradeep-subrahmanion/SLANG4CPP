...
Procedure::Procedure(string _name, vector<SymbolInfo *> _formals, vector<Statement *>stats, SymbolInfoTable *_locals,
    TypeInfo _type)
{
  name = _name;
  formals = _formals;
...
SymbolInfo * Procedure::execute(Runtime_Context *ctx, vector<SymbolInfo *>actuals)
{
...
SymbolInfo * Tmodule::execute(Runtime_Context *ctx, vector<SymbolInfo *>_actuals)
{
