...
{
  return new Procedure(proc_name, formals, statements, ctx->get_symboltable(),type);
}
...
void TmoduleBuilder::add_function_prototypes(string _name, TypeInfo _ret_type, vector<TypeInfo> _formals)
{
    FunctionInfo *info = new FunctionInfo(_name, _ret_type, _formals);
    protos.push_back(info);
}

bool TmoduleBuilder::check_function_prototypes(string _name, TypeInfo _ret_type, vector<TypeInfo> _formals)
{
  for(int i=0;i<protos.size();++i) {
    FunctionInfo *info = protos.at(i);
